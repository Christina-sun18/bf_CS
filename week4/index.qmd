---
title: "Week 4: The forecaster's toolbox"
---

```{r}
week <- 4
source(here::here("course_info.R"))
```

## What you will learn this week

* Four benchmark forecasting methods that we will use for comparison
* Fitted values, residuals
* Forecasting with transformations

## Exercises (on your own or in tutorial)

Complete Exercise 2, 3, 10 from [Section 3.7 of the book](https://otexts.com/fpp3/decomposition-exercises.html)

## Exercise solutions

<!-- ::: {.callout appearance="minimal"} -->
<!-- <i class="bi bi-download"></i> [Week 4 Tutorial Solution.html](Week4.html){download="Week 4 Tutorial Solution.html"} -->
<!-- ::: -->


## Pre-seminar activities

Read [Chapter 5 of the textbook](https://otexts.com/fpp3/toolbox.html) and watch all embedded videos

```{r}
#| output: asis
show_slides(week)
```

## Lectorial activities

```{r}
#| child: activities.md
```


## Seminar code

::: {.callout appearance="minimal"}
<i class="bi bi-download"></i> [week4_incomplete.R](week4_incomplete.R){download="week4_incomplete.R"}
:::



```{r, echo=FALSE}
#| output: asis

show_assignments(week)
```
